...
(function (EditingTools) {
    ...
    EditingTools.setProject = setProject;
    function setAnimation(spriteAnimation) {
    ...
    function setScaleHandler() {
        ...
        html.scaleControlInput.oninput = function (evt) {
            let val = evt.target.valueAsNumber;
            if (!val || val < 10 || val > 6400)
                return;
            let scale = (val / 100.0);
            let oldScale = 1 / EditingTools.canvasScale;
            EditingTools.canvasScale = 1 / scale;
    ...
    function getSpriteCordsFromEvt(evt) {
        let rect = html.mainCanvas.getBoundingClientRect();
        let x = (evt.clientX - rect.left) * EditingTools.canvasScale - RTools.canvasPadding;
        let y = (evt.clientY - rect.top) * EditingTools.canvasScale - RTools.canvasPadding;
        return { x, y };
    }
    function getAboluteFramePosition(ind) {
     ...
     *   Name Editing stage.
     *******************************************************************************/
    function addAnimationListElement(nextStage) {
    ...
    EditingTools.addAnimationListElement = addAnimationListElement;
    /********************************************************************************
     *    Base box set up stage
     ********************************************************************************/
    function getBaseBoxSizesDialog(frames) {
    ...
    EditingTools.getBaseBoxSizesDialog = getBaseBoxSizesDialog;
    function getBaseBoxInputHandler(frames, direction) {
        /* Handlers */
        function baseBoxResize(input, property) {
    ...
    function getDragingOnMouseDownHandler(frames) {
        ...
        let func = (evt) => {
            ...
            let lastX = evt.x * EditingTools.canvasScale;
            let lastY = evt.y * EditingTools.canvasScale;
            document.onmousemove = (evt) => {
                let e = {
                    x: evt.clientX * EditingTools.canvasScale,
                    y: evt.clientY * EditingTools.canvasScale
                };
                let { baseBox } = frames;
        ...
        function resizeDraging(evt, side) {
            flags.baseBoxResizeDraging = true;
            let lastX = evt.clientX * EditingTools.canvasScale;
            let lastY = evt.clientY * EditingTools.canvasScale;
            document.onmousemove = (evt) => {
                let e = {
                    x: evt.clientX * EditingTools.canvasScale,
                    y: evt.clientY * EditingTools.canvasScale
    ...
    function getBaseBoxIsSetHandler(nextStage, dragingIdicator) {
        let baseBoxSet = () => {
        ...
        return function (e) {
            if (e.key == 'a') {
                ...
                document.body.onmousemove = (e) => {
                    ...
                    let y = e.clientY * EditingTools.canvasScale;
    ...
    EditingTools.getFrameAdderHandler = getFrameAdderHandler;
    let selectedFrameHandlers = {};
    ...
    EditingTools.getFrameSelectionHandler = getFrameSelectionHandler;
    ...
    function selectedSetHandlers(selectedInd) {
        ...
        document.body.addEventListener("keydown", selectedFrameHandlers.keyDown);
    ...
    function getSelectedFrameDragStart(frameIndex) {
        return function (evt) {
            if (html.mainCanvas.style.cursor != "pointer")
                return false;
            ...
            selectedFrameHandlers.cropDrag = (evt) => {
                let { crop } = curAnimation.frames.frameDeltas[frameIndex];
                let { baseBox } = curAnimation.frames;
                let x = evt.clientX * EditingTools.canvasScale;
                let y = evt.clientY * EditingTools.canvasScale;
                if (side.left) {
            ...
            document.addEventListener("mousemove", selectedFrameHandlers.cropDrag);
            function endDrag() {
                document.removeEventListener("mousemove", selectedFrameHandlers.cropDrag);
    ...
    function updateFramesBorders(side, diff, curInd) {
        const { frames } = curAnimation;
    ...
    function getSelectedFrameKeyHandler(ind) {
        return function (evt) {
            switch (evt.key) {
                case "Enter":
                case "Escape":
    ...
    function selectedFrameMove(ind, direction) {
        let fd = curAnimation.frames.frameDeltas;
    ...
    function selectedFrameRemove(ind) {
        if (ind < 0)
            return;
     ...
     * are not selected. */
    function findSelectedFrameIndex(evt) {
        let cords = getSpriteCordsFromEvt(evt);
