class SpriteAnimation {
    constructor(parent, isLoaded = false) {
        this.parent = parent;
     ...
     */
    stateBasedFrameAdder(frameNum, xPos, yPos, stickToAxis = true, update = false) {
        const fd = this.frames.frameDeltas;
        // Get static 'temp' object if exist or 'undefined' otherwise.
        let temp = this.stateBasedFrameAdder.temp;
        if (!temp && frameNum < 0)
            return;
        if (frameNum < 0)
        ...
        if (!temp) {
            ...
            // Set static varaible
            this.stateBasedFrameAdder.temp = temp;
        }
        ...
        if (update) {
            ...
            this.stateBasedFrameAdder.temp = null;
        }
        RTools.drawFrameBoxes(this.frames);
    }
